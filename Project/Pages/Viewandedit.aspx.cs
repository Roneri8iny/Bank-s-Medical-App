...
public partial class Pages_Viewandedit : System.Web.UI.Page
{
    ...
    {
        var query = db.MedicalFields.AsQueryable();

        if (!string.IsNullOrEmpty(searchTerm))
        {
            query = query.Where(mf => mf.Username.Contains(searchTerm) || mf.Email.Contains(searchTerm));
        }

        AccountsGridView.DataSource = query.Select(mf => new
        {
            mf.MFID,
            mf.Username,
            mf.Email,
    ...
    private void LoadEditModal(int mfid)
    {
    ...
    private void DeleteAccount(int mfid)
    {
        try
        {
